...
vector<shared_ptr<Game_Object>>& Wave::create(sf::Vector2f const& player_pos) {
    ...
    for(int i{0}; i < wave_number + 1; ++i){
        ...
        if(spawning_position==1) {          //Above Screen
            pos = {static_cast<float>(random_int(-150, WIDTH)),
                   static_cast<float>(random_int(-200, 0))};
        }
        else if(spawning_position==2) {      //Below Screen
        pos = {static_cast<float>(random_int(-150, WIDTH)),
               static_cast<float>(random_int(HEIGHT, HEIGHT + 200))};
        }
        else if(spawning_position==3){      //Left of Screen
            pos = {static_cast<float>(random_int(-300, -150)),
                   static_cast<float>(random_int(-200, HEIGHT))};
        }
        else{                               //Right of Screen
            pos = {static_cast<float>(random_int(WIDTH, WIDTH + 200)),
                   static_cast<float>(random_int(-200, HEIGHT))};
        }
